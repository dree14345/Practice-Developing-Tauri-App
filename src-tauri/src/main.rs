...
// CREATE
#[tauri::command]
async fn create_item(state: State<'_, AppState>, name: String, description: Option<String>) -> Result<Item, String> {
    sqlx::query_as!(Item, "INSERT INTO items(name, description) VALUES($1, $2) RETURNING id, name, description", name, description).fetch_one(&state.db).await.map_err(|e| e.to_string())  
}
// READ (all)
#[tauri::command]
...
// UPDATE
#[tauri::command]
async fn update_item(state: State<'_, AppState>, id: i32, name: String, description: Option<String>) -> Result<Item, String> {
